...
class IncrementalTrainer(Trainer):
    ...
    def train_on_audio(self, fn: str):
        ...
        for i, chunk in enumerate(chunk_audio(audio, self.args.chunk_size)):
            ...
            print('chunk_size: ', self.args.chunk_size)
            print('\r' + str(i * 100. / num_chunks) + '%', end='', flush=True)
            # size of audio_buffer is 24000 samples (16000 sample/s x 1.5 s)
            ...
            if conf > 0.5:
                ...
                name = join(self.args.folder, 'test' if save_test else '', 'not-wake-word', 'generated', name)
    ...
    def run(self):
        ...
        print('Validation: ', self.args.no_validation)
